...
# lipid_types <- c( "TAG1" = "TAG1", "TAG2" = "TAG2")
...
PCA_names_to_parse <- c('5DN1','5DN2','5DN3','5DN4',"5DN5",
                        '5GFP1','5GFP2','5GFP3','5GFP4',"5GFP5",
                        '7C1','7C2','7C3','7C4',"7C5",
                        '7E1','7E2','7E3','7E4',"7E5",
                        '42C1','42C2','42C3','42C4',"42C5",              ##These are controls compare C to E
                        '42E1','42E2','42E3','42E4',"42E5",   
                        '40DN1','40DN2','40DN3','40DN4',"40DN5",
                        '40GFP1','40GFP2','40GFP3','40GFP4',"40GFP5",'40DP1','40DP2','40DP3','40DP4',"40DP5",
                        "SolventBlank1")


####No Neurons
PCA_names_to_parse_No_Neurons <- c('5GFP1','5GFP2','5GFP3','5GFP4',"5GFP5",
                        '7C1','7C2','7C3','7C4',"7C5",
                        '7E1','7E2','7E3','7E4',"7E5",
                        '42C1','42C2','42C3','42C4',"42C5",              ##These are controls compare C to E
                        '42E1','42E2','42E3','42E4',"42E5",
...
#                '5GFP1','5GFP2','5GFP3','5GFP4',"5GFP5",
#                '7C1','7C2','7C3','7C4',"7C5",
#                '40C1','40C2','40C3','40C4',"40C5",              ##These are controls compare C to E
#                '40DN1','40DN2','40DN3','40DN4',"40DN5",
#                '40DP1','40DP2','40DP3','40DP4',"40DP5",
#                '40GFP1','40GFP2','40GFP3','40GFP4',"40GFP5",
#                "SolventBlank1")
...
###5 vs 40 N
experiment_1_names <- c('40DN1','40DN2','40DN3','40DN4',"40DN5",'5DN1','5DN2','5DN3','5DN4',"5DN5","SolventBlank1")


###5 vs 40 GFP
experiment_2_names <- c('40GFP1','40GFP2','40GFP3','40GFP4',"40GFP5",'5GFP1','5GFP2','5GFP3','5GFP4',"5GFP5","SolventBlank1")


# 
# ##40 DP vs GFP
experiment_3_names <- c('42C1','42C2','42C3','42C4',"42C5",
                        '7C1','7C2','7C3','7C4',"7C5",
                        "SolventBlank1")
# ##40 DP vs DN
...
read_lipids <- function(prefix, marker, exp_names, types, add_Blank = NULL) {
  lapply(names(types), function(type){
    ...
    all_names <- lapply(exp_names, function(n) {
      ...
      read_xlsx(file_name) %>%
        dplyr::rename(lipid = `Lipid name`) %>%
        dplyr::rename(!!n := `Total Intensity`) %>%
        mutate(type = type) %>%
        na.omit
    })
    ...
    if (!is.null(add_Blank)) {
      Blank = read_xlsx(paste0(dir_name, add_Blank, suffix)) %>%
        dplyr::rename(lipid = `Lipid name`) %>%
        dplyr::rename(!!add_Blank := `Total Intensity`) %>%
        mutate(type = type) %>%
        na.omit
...
write_summary_and_results <- function(tbl, df, name) {
  ...
  results %>%
    group_by(type) %>%
    summarise(Down_regulated = sum(logFC < 0 & FDR < 0.1),
              Up_regulated = sum(logFC > 0 & FDR < 0.1)) %>%
...
get_DE_lipids <- function(counts, design_mat, gr, contrasts, p.value = .1) {
  dls <-
    counts %>%
...
get_DE_lipids3 <- function(counts, design_mat, gr, contrasts, p.value = .1) {
  dls <-
    counts %>%
    perform_analysis_raw(design_mat, gr) %>%
    calculate_significance(contrasts) %>%
...
make_pca_plot3 <- function(tp, design_mat, gr, contrasts,
                           title = "PCA plot",
                           ellipse = T, var.axes = F,
                           labels = T) {
  ...
  tp_edger <-
    tp %>%
    get_DE_lipids3(design_mat, gr, contrasts)
...
# 
cells_lipid_expr_1 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_1,sep=''))
ggsave(paste("plots2/PCA__FDR_",title_for_plot_1,".pdf",sep=''))

cells_lipid_expr_2 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_2,sep=''))
ggsave(paste("plots2/PCA_",title_for_plot_2,".pdf",sep=''))
#
# 
# cells_lipid_expr_3 %>%
#   make_pca_plot(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_3,sep=''))
# ggsave(paste("plots2/PCA_",title_for_plot_3,".pdf",sep=''))
# # 
# # 
# # 
# cells_lipid_expr_4 %>%
#   make_pca_plot2(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_4,sep=''))
# ggsave(paste("plots2/PCA_",title_for_plot_4,".pdf",sep=''))
# 

cells_lipid_expr_5 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_5,sep=''))
ggsave(paste("plots2/PCA_FDR_",title_for_plot_5,".pdf",sep=''))
...
# 
cells_lipid_expr_7 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_7,sep=''))
ggsave(paste("plots2/PCA_FDR_",title_for_plot_7,".pdf",sep=''))

# 
# 
# cells_lipid_expr_8 %>%
#   make_pca_plot(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_8,sep=''))
# ggsave(paste("plots2/PCA_",title_for_plot_8,".pdf",sep=''))
# 
# 
# 
cells_lipid_expr_9 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_9,sep=''))
ggsave(paste("plots2/PCA_",title_for_plot_9,".pdf",sep=''))

# 
# cells_lipid_expr_10 %>%
#   make_pca_plot2(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_10,sep=''))
# ggsave(paste("plots2/PCA_",title_for_plot_10,".pdf",sep=''))
# 
# 
# 
# cells_lipid_expr_11 %>%
#   make_pca_plot(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_11,sep=''))
# ggsave(paste("plots2/PCA_",title_for_plot_11,".pdf",sep=''))


cells_lipid_expr_12 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_12,sep=''))
ggsave(paste("plots2/PCA_FDR_",title_for_plot_12,".pdf",sep=''))

cells_lipid_expr_13 %>%
  make_pca_plot3(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_13,sep=''))
ggsave(paste("plots2/PCA_FDR_",title_for_plot_13,".pdf",sep=''))

# 
# cells_lipid_expr_14 %>%
#   make_pca_plot2(design_expr_5, gr_expr_5, contrasts_expr_5, paste("PCA_Plot_",title_for_plot_14,sep=''))
# ggsave(paste("plots2/PCA_",title_for_plot_14,".pdf",sep=''))
# 
# 
